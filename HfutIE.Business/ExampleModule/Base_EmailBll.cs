    ...
    public class Base_EmailBll : RepositoryFactory<Base_Email>
    {
        ...
        public DataTable GetCountEmail(string UserId)
        {
            ...
            strSql.Append(@"SELECT  SUM(CASE a.IsRead
                                          ...
                                          ELSE 0
                                        END) AS UnRead ,	   	   
                                    MAX(EA.Draft) AS Draft ,		                                           
                                    MAX(EA.Sended) - MAX(ISNULL(EA.DeleteMark, 0))-MAX(ISNULL(EA.DropMark, 0)) AS Sended ,				
                                    SUM(CASE a.DeleteMark WHEN 1 THEN 1 ELSE 0 END) + MAX(ISNULL(EA.DeleteMark, 0)) AS DELETED 
                            FROM    Base_Email e
