...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_FaultBaseBll : RepositoryFactory<BBdbR_FaultBase>
    {
        #region 全局变量定义区
        //定义本页面主要操作的表的表名，称为主表
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_FaultBase entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_FaultBase entity) //===复制时需要修改===
        {
            entity.Enabled = "1";
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_FaultBase> listEntity = new List<BBdbR_FaultBase>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BBdbR_FaultBase entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
        ...
        public List<BBdbR_FaultBase> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<BBdbR_FaultBase> dt;
            if (Condition == "all")
            {
                sql = @"select * from " + tableName + " where Enabled = '1' order by FaultCd";
                dt = Repository().FindListBySql(sql.ToString());
            }
            else
            {
                //根据条件查询
                sql = @"select * from " + tableName + " where  " + Condition + " like  '%" + keywords + "%' and Enabled = '1' order by FaultCd";
                dt = Repository().FindListBySql(sql.ToString());
            }
            return dt;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<BBdbR_FaultBase> GetFaultList()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  * FROM  BBdbR_FaultBase where Enabled=1 ");
            List<BBdbR_FaultBase> dt = Repository().FindListBySql(strSql.ToString());
            return dt;
        }
        #endregion
