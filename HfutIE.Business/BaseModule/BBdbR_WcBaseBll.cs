    ...
    public class BBdbR_WcBaseBll : RepositoryFactory<BBdbR_WcBase>
    {
        ...
        public DataTable GetTree()
        {
            ...
            strSql.Append(@"select c.WorkshopId As keys,
                                   ...
                                   '0' as parentId,
                                   '0' as sort             
                                   from BBdbR_WorkshopBase c where 1 = '1'");
            //工段节点
            strSql.Append(@"union select 
                                    ...
                                    '1' as sort 
                                    from BBdbR_WorkSectionBase b,BBdbR_WorkshopBase c where b.WorkshopId = c.WorkshopId and 1= '1'  ");
           //产线节点
            strSql.Append(@" union select    
                                    a.PlineId AS keys,
                                    a.PlineCd AS code,
        ...
        public DataTable GetList(string areaId,string parentId,string sort, ref JqGridParam jqgridparam) //===复制时需要修改===
        {
            ...
            {
                sql = "select a.*,b.PlineCd as PlineCd,b.PlineNm as PlineNm from " + tableName + " a join BBdbR_PlineBase b on a.PlineId=b.PlineId where a.Enabled=1 and b.Enabled=1";     //===复制时需要修改===
            }
            else
            {
                if (parentId != "0")
                {
                    if (sort == "0")
                    {
                        sql = "select a.*,b.PlineCd as PlineCd,b.PlineNm as PlineNm from " + tableName + " a join BBdbR_PlineBase b on a.PlineId=b.PlineId join BBdbR_WorkSectionBase c on b.WorkSectionId=c.WorkSectionId where a.Enabled=1 and b.Enabled=1 and c.Enabled=1 and c.WorkSectionId='" + areaId + "'";     //===复制时需要修改===
                    }
                    else if(sort == "1")
                    {                     
                        sql = "select a.*,b.PlineCd as PlineCd,b.PlineNm as PlineNm from " + tableName + " a join BBdbR_PlineBase b on a.PlineId=b.PlineId join BBdbR_WorkSectionBase c on b.WorkSectionId=c.WorkSectionId where a.Enabled=1 and b.Enabled=1 and c.Enabled=1 and c.WorkSectionId='" + areaId + "'";     //===复制时需要修改===
                    ...
                    {
                        sql = "select a.*,b.PlineCd as PlineCd,b.PlineNm as PlineNm from " + tableName + " a join BBdbR_PlineBase b on a.PlineId=b.PlineId where a.Enabled=1 and b.Enabled=1 and a.PlineId='"+ areaId + "'";     //===复制时需要修改===要修改===
                    }
                }
                else
                {
                    sql = "select a.*,b.PlineCd as PlineCd,b.PlineNm as PlineNm from " + tableName + " a join BBdbR_PlineBase b on a.PlineId=b.PlineId join BBdbR_WorkSectionBase c on b.WorkSectionId=c.WorkSectionId join BBdbR_WorkshopBase d on c.WorkshopId=d.WorkshopId where a.Enabled=1 and b.Enabled=1 and c.Enabled=1 and d.Enabled=1 and d.WorkshopId='"+ areaId + "'";     //===复制时需要修改===
                }
            }
        ...
        public int hasChildNode(string WcCd)
        {
            string sql = @"select * from BBdbR_PostBase where WcCd = '" + WcCd + "'";
            DataTable count = Repository().FindTableBySql(sql);
            int a = count.Rows.Count;
            return a;
        }
        ...
        public List<BBdbR_WcBase> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            if (Condition == "all")
            {
                sql = @"select * from " + tableName + " where Enabled = 1 order by WcCd asc";
