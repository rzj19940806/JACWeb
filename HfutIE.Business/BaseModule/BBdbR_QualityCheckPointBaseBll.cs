...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_QualityCheckPointBaseBll : RepositoryFactory<BBdbR_QualityCheckPointBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_QualityCheckPointBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_QualityCheckPointBase> repository_User = new RepositoryFactory<BBdbR_QualityCheckPointBase>();
        #endregion
        ...
        public List<BBdbR_QualityCheckPointBase> GetPageList(JqGridParam jqgridparam) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_QualityCheckPointBase entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_QualityCheckPointBase entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_QualityCheckPointBase> listEntity = new List<BBdbR_QualityCheckPointBase>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BBdbR_QualityCheckPointBase entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
        ...
        public List<BBdbR_QualityCheckPointBase> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<BBdbR_QualityCheckPointBase> dt;
            if (Condition == "all")
            {
        ...
        #region 获取最大顺序号
        public int GetMaxNo(string QueueNo)
        {
            StringBuilder str = new StringBuilder();
            str.Append(@"select max(" + QueueNo +") from " + tableName + " where Enabled = '1'");
            Object obj = Repository().FindMaxBySql(str.ToString());//这里用FindMaxBySql的目的是获取最大版本号
