...
namespace HfutIE.Business
{
    ...
    /// <author>
    ///		<name>she</name>
    ///		<date>2021.09.30 15:14</date>
    /// </author>
    /// </summary>
    public class BFacR_ShiftTimeConfigBll : RepositoryFactory<BFacR_ShiftTimeConfig>
    {

        public List<BFacR_ShiftTimeConfig> GetConfigList1(string keywords, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            if (keywords != "")
            {
                sql = @"select a.* from BFacR_ShiftTimeConfig a join BFacR_ShiftBase b on a.ShiftId=b.ShiftId where a.Enabled=1 and b.Enabled=1 and b.ShiftId='" + keywords + "'";
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BFacR_ShiftTimeConfig entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BFacR_ShiftTimeConfig entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BFacR_ShiftTimeConfig> listEntity = new List<BFacR_ShiftTimeConfig>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BFacR_ShiftTimeConfig entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
