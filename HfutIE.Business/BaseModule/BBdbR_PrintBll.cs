...
namespace HfutIE.Business
{
    ...
    public class BBdbR_PrintBll : RepositoryFactory<BBdbR_Print>
    {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_Print entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_Print entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_Print> listEntity = new List<BBdbR_Print>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                string sql = @"select * from " + tableName + " where  Id  = '" + keyValue + "'";
                BBdbR_Print entity = Repository().FindEntityBySql(sql);
                //BBdbR_Print entity2 = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
