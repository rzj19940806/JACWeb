...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_SupplierBaseBll : RepositoryFactory<BBdbR_SupplierBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_SupplierBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_SupplierBase> repository_User = new RepositoryFactory<BBdbR_SupplierBase>();
        #endregion
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_SupplierBase entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_SupplierBase entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_SupplierBase> listEntity = new List<BBdbR_SupplierBase>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BBdbR_SupplierBase entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
                entity.Enabled = "0";//将该实体的IsAvailable属性改为false
                entity.Modify(entity.SupplierId);
        ...
        public List<BBdbR_SupplierBase> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<BBdbR_SupplierBase> dt;
            if (Condition == "all")
            {
        ...
        /// <returns></returns>
        //public List<BBdbR_SupplierBase> GetPlanList()
        //{
        //    StringBuilder strSql = new StringBuilder();
        //    strSql.Append(@"SELECT  * FROM  BBdbR_SupplierBase where Enabled = '1' and MatCatg = '产品'");
        //    List<BBdbR_SupplierBase> dt = Repository().FindListBySql(strSql.ToString());
        //    return dt;
        //}
        /// <summary>
        /// 查询检查项目
        ...
        /// <param name="CheckId"></param>
        /// <returns></returns>
        public BBdbR_SupplierBase GetPlanList1(string SupplierId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  * FROM  BBdbR_SupplierBase where Enabled = '1'  and SupplierId='" + SupplierId + "'");
            List<BBdbR_SupplierBase> dt = Repository().FindListBySql(strSql.ToString());
            if (dt.Count > 0)
            {
                return dt[0];
        ...
        #region 查询配置表中不存在的检查项目信息
        //public List<BBdbR_SupplierBase> GetCheckItemList(string CarPartId)
        //{
        //    StringBuilder strSql = new StringBuilder();
        //    strSql.Append(@"SELECT  * FROM  BBdbR_SupplierBase where Enabled = '1'  and SupplierId not in (SELECT SupplierId  FROM  BBdbR_CarPartQuaCheckItemConfig where Enabled = '1' and CarPartId = '" + CarPartId + "' )");
        //    List<BBdbR_SupplierBase> dt = Repository().FindListBySql(strSql.ToString());
        //    return dt;
