...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class MBB_BD_PRODUBll : RepositoryFactory<MBB_BD_PRODU>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "MBB_BD_PRODU";//===复制时需要修改===
        ...
        public List<MBB_BD_PRODU> GetPageList(JqGridParam jqgridparam) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(MBB_BD_PRODU entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(MBB_BD_PRODU entity) //===复制时需要修改===
        {
            entity.Enabled = "1";
        ...
        public int Delete(string[] array)
        {
            ...
            List<MBB_BD_PRODU> listEntity = new List<MBB_BD_PRODU>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                MBB_BD_PRODU entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
                entity.Enabled = "0";//将该实体的IsAvailable属性改为false
                listEntity.Add(entity);
            }
            return Repository().Update(listEntity);//删数据库
        }
        #endregion
        ...
        public List<MBB_BD_PRODU> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<MBB_BD_PRODU> dt;
            if (Condition == "all")
            {
