...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_CarPartBaseBll : RepositoryFactory<BBdbR_CarPartBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_CarPartBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_CarPartBase> repository_User = new RepositoryFactory<BBdbR_CarPartBase>();
        #endregion
        ...
        public List<BBdbR_CarPartBase> GetPageList(JqGridParam jqgridparam) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_CarPartBase entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_CarPartBase entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            {
                //===复制时需要修改===
                BBdbR_CarPartBase entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
                //entity.Enabled = "0";//将该实体的IsAvailable属性改为false
                //entity.Modify(keyValue);
                if (entity.CarPartImg !=null)
                {
                    sql.Append(@"update " + tableName + " set CarPartImg = @content,Enabled = '0' where CarPartId = '" + keyValue + "'");
                    var dbparameter = new SqlParameter("@content", entity.CarPartImg);
                    isok += Repository().ExecuteBySql(sql, new[] { dbparameter });
                }
                else
                {
                    entity.Enabled = "0";
                    isok += Repository().Update(entity);
                }
        ...
        public List<BBdbR_CarPartBase> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<BBdbR_CarPartBase> dt;
            if (Condition == "all")
            {
        ...
        #region 根据ID获取数据
        public DataTable GetCarInfor(string KeyValue)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"select * from BBdbR_CarPartBase where CarPartId='" + KeyValue + "' and Enabled = '1'");
        ...
        #region 插入图片
        public int InsertPicture(string CarPartCd, byte[] image, string name) //===复制时需要修改===
        {
            ...
            sql.Append(@"update " + tableName + " set CarPartImg = @content where CarPartCd = '" + CarPartCd + "'");
            var dbparameter = new SqlParameter("@content", image);
            return Repository().ExecuteBySql(sql, new[] { dbparameter });
        }
        #endregion

        #region 查询配置表中不存在的物料信息
        public List<BBdbR_CarPartBase> GetCarPartList(string QualityCheckModelId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  * FROM  BBdbR_CarPartBase where Enabled = '1'  and CarPartId not in (SELECT CarPartId  FROM  BBdbR_QuaCheckModelCarPartConfig where Enabled = '1' and QualityCheckModelId = '" + QualityCheckModelId + "' )");
            List<BBdbR_CarPartBase> dt = Repository().FindListBySql(strSql.ToString());
            return dt;
        ...
        /// <param name="CheckId"></param>
        /// <returns></returns>
        public BBdbR_CarPartBase GetPlanList1(string CarPartId)
        {
            StringBuilder strSql = new StringBuilder();
