...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class QAS_TorsionItemBaseBll : RepositoryFactory<QAS_TorsionItemBase>
    {
        #region
        #endregion
        ...
        #region 全局变量定义区
        //定义本页面主要操作的表的表名，称为主表
        string tableName = "QAS_TorsionItemBase";
        ...
        /// <returns></returns>
        public List<QAS_TorsionItemBase> GetPageList(string PropertyName, string PropertyValue)
        {
            List<QAS_TorsionItemBase> dt;
            string sql = "";
            if (PropertyName == "")
            {
                sql = "select * from " + tableName + " where 1=1 order by TORItemID";
                dt = Repository().FindListBySql(sql);
            }
            else
            {
                //根据条件查询
                sql = "select * from " + tableName + " where " + PropertyName + " like  '%" + PropertyValue + "%' order by TORItemID";
                dt = Repository().FindListBySql(sql);
            }
            return dt;
        }
        ...
        /// <summary>
        /// 导入Tor扭力检测项信息Excell数据
        /// </summary>
        /// <param name="Id">主键</param>
        /// <returns></returns>
        public int ImportExcelTor(string moduleId, DataTable dt, string[] cartypes, out DataTable Result)
        {
            int IsOk = 0;//导入状态
            bool CheckEx = false;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append("delete QAS_TorsionItemBase where 1=1");
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            #endregion
                            //if (entity["order_category"].ToString() == "项目")
                            //{
                            //    database.Insert(base_excellimport.ImportTable, entity, isOpenTrans);
                            //}
                            //database.Insert("orders_all", entity, isOpenTrans);
                            database.Insert(base_excellimport.ImportTable, entity, isOpenTrans);
                            rowNum++;
                        }
