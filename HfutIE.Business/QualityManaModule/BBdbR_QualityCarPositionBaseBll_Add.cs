...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_QualityCarPositionBaseBll_Add : RepositoryFactory<BBdbR_QualityCarPositionBase_Add>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_QualityCarPositionBase_Add";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_QualityCarPositionBase_Add> repository_User = new RepositoryFactory<BBdbR_QualityCarPositionBase_Add>();
        #endregion
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_QualityCarPositionBase_Add entity) //===复制时需要修改===
        {
        ...
        {
            string sql = @"select * from BBdbR_QualityCarPositionGroupBase_Add where Enabled = '1' and CarPositionId = '" + KeyValue + "'";
            DataTable count = Repository().FindTableBySql(sql);
            int a = count.Rows.Count;
            return a;
        }
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_QualityCarPositionBase_Add entity) //===复制时需要修改===
        {
        ...
        public int Delete(string KeyValue)
        {
            ...
            List<BBdbR_QualityCarPositionBase_Add> listEntity = new List<BBdbR_QualityCarPositionBase_Add>(); //===复制时需要修改===

            //===复制时需要修改===
            BBdbR_QualityCarPositionBase_Add entity = Repository().FindEntity(KeyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
        ...
        public List<BBdbR_QualityCarPositionBase_Add> GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            List<BBdbR_QualityCarPositionBase_Add> dt;
            if (Condition == "all")
            {
        ...
        /// <param name="CheckId"></param>
        /// <returns></returns>
        public BBdbR_QualityCarPositionBase_Add GetPlanList1(string CarPartId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  * FROM  BBdbR_CarPartBase where Enabled = '1' and CarPartId='" + CarPartId + "'");
            List<BBdbR_QualityCarPositionBase_Add> dt = Repository().FindListBySql(strSql.ToString());
            return dt[0];
        }
        #endregion
