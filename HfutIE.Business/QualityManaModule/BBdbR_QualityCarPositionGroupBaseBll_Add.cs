...
namespace HfutIE.Business
{
    ...
    /// </author>
    /// </summary>
    public class BBdbR_QualityCarPositionGroupBaseBll_Add : RepositoryFactory<BBdbR_QualityCarPositionGroupBase_Add>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_QualityCarPositionGroupBase_Add";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_QualityCarPositionGroupBase_Add> repository_User = new RepositoryFactory<BBdbR_QualityCarPositionGroupBase_Add>();
        #endregion
        ...
        public DataTable GetTree(string type)
        {
            ...
            strSql.Append(@" select    
                                    ...
                                    '0' AS parentId,
                                    '0' as sort 
                                    from BBdbR_QualityCarPositionBase_Add where Enabled = '1'and type='CH' order by code asc");
            }
            else
            {
                strSql.Append(@" select    
                                    ...
                                    '0' AS parentId,
                                    '0' as sort 
                                    from BBdbR_QualityCarPositionBase_Add where Enabled = '1'and type='TZ'  order by code asc");
            }
        ...
        /// <returns></returns>
        public DataTable GetList(string type,string areaId, string parentId, string CarPositionGroupCd, string CarPositionGroupNm, ref JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            DataTable dt = new DataTable();
            if (string.IsNullOrEmpty(areaId) && string.IsNullOrEmpty(parentId))
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase_Add b on a.CarPositionId=b.CarPositionId where a.Enabled=1 and b.Enabled = '1' and b.type='"+type+"'";     //===复制时需要修改===
            }
            else if (areaId != "all")
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase_Add b on a.CarPositionId=b.CarPositionId where a.Enabled=1 and a.CarPositionId = '" + areaId + "'and  b.type='" + type + "' ";     //===复制时需要修改===
            }
            else
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase_Add b on a.CarPositionId=b.CarPositionId where a.Enabled=1 and b.type='" + type + "'";     //===复制时需要修改===
            }
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_QualityCarPositionGroupBase_Add entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_QualityCarPositionGroupBase_Add entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_QualityCarPositionGroupBase_Add> listEntity = new List<BBdbR_QualityCarPositionGroupBase_Add>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BBdbR_QualityCarPositionGroupBase_Add entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
        ...
        public DataTable GetPageListByCondition(string keywords, string Condition, JqGridParam jqgridparam) //===复制时需要修改===
        {
            ...
            if (Condition == "all")
            {
                sql = @"select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase_Add b on a.CarPositionId=b.CarPositionId where a.Enabled = '1'";
                dt = Repository().FindTableBySql(sql.ToString(), false);
            }
            else
            {
                //根据条件查询
                sql = @"select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase_Add b on a.CarPositionId=b.CarPositionId where a.Enabled = '1' and a." + Condition + " like  '%" + keywords + "%'";
                dt = Repository().FindTableBySql(sql.ToString(), false);
            }
            return dt;
        }
