...
namespace HfutIE.Business
{
    ...
    public class BBdbR_QualityCarPositionGroupBaseBll : RepositoryFactory<BBdbR_QualityCarPositionGroupBase>
    {
        ...
        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_QualityCarPositionGroupBase";//===复制时需要修改===
        ...
        /// <returns></returns>
        public DataTable GetList(string areaId, string parentId, string CarPositionGroupCd, string CarPositionGroupNm, ref JqGridParam jqgridparam) //===复制时需要修改===
        {
            string sql = "";
            DataTable dt = new DataTable();
            if (string.IsNullOrEmpty(areaId) && string.IsNullOrEmpty(parentId))
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase b on a.CarPositionId=b.CarPositionId where a.Enabled=1 and b.Enabled = '1' ";     //===复制时需要修改===
            }
            else if(areaId != "all")
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase b on a.CarPositionId=b.CarPositionId where a.Enabled=1 and a.CarPositionId = '" + areaId + "'  ";     //===复制时需要修改===
            }
            else
            {
                sql = "select a.*,b.CarPositionCd AS CarPositionCd,b.CarPositionNm AS CarPositionNm from " + tableName + " a join BBdbR_QualityCarPositionBase b on a.CarPositionId=b.CarPositionId where a.Enabled=1 ";     //===复制时需要修改===
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Update(BBdbR_QualityCarPositionGroupBase entity) //===复制时需要修改===
        {
        ...
        //返回值为1，或者0
        //1表示操作成功，0表示操作失败
        public int Insert(BBdbR_QualityCarPositionGroupBase entity) //===复制时需要修改===
        {
        ...
        public int Delete(string[] array)
        {
            ...
            List<BBdbR_QualityCarPositionGroupBase> listEntity = new List<BBdbR_QualityCarPositionGroupBase>(); //===复制时需要修改===
            foreach (string keyValue in array)
            {
                //===复制时需要修改===
                BBdbR_QualityCarPositionGroupBase entity = Repository().FindEntity(keyValue);//根据主键（keyValue）在数据库中查找实体 //===复制时需要修改===
