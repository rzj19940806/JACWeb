...
{
    public abstract class DbHelperFeed
    {
        protected static string connectionString = "Server=172.26.35.2;Initial Catalog=feed_data;User ID=sa;Password=sa";//正式库
        ...
        //protected static string connectionString = "Server=.;Initial Catalog=DCT_ACTIVE;User ID=sa;Password=sa123";//本地数据库
        //protected static string connectionString = "Server=192.168.0.111;Initial Catalog=DCT_ACTIVE;User ID=sa;Password=sa123";//其它库
        public DbHelperFeed()
        {
        }
        ...
        public static string GetStrValue(string TableName, string ColumnName, string Value, string GoalColumn)
        {
            ...
            DataTable dt = OpenTable("select " + GoalColumn + " from " + TableName + " where " + ColumnName + " = '" + Value + "'");
            if (dt.Rows.Count > 0)
            {
        ...
        /// 物流票据打印时计划订单查询条件，返回的是已经排产的订单信息
        /// </summary>
        /// <param name="TableName">表名</param>
        /// <param name="ColumnName">列名</param>
        /// <param name="KeyValue">值</param>
        /// <returns></returns>
        public static DataTable GetObjectDataLikeKeyValueLogic(string TableName, string ColumnName, string KeyValue)
        {
            DataTable ReturnTable = new DataTable();
            if (KeyValue.ToString() != "" || KeyValue != null)
            {
                string SqlStr = "select * from " + TableName + " where " + ColumnName + " like '%" + KeyValue + "%' and schedule_state = '排产完成' and done_condition != '执行完毕'";
                ReturnTable = OpenTable(SqlStr);
            }
            return ReturnTable;
        }
