/**
 * jQuery HfutIEUI 4.1
 *安徽华元智控科技有限公司 Copyright © IMSClientBS 2014
 */
//样式
$(function () {
    readyIndex();
...
function Loadlayout() {
    if ($('.layout').length > 0) {
        $("#layout").splitter({
            type: "v",
            outline: true,
            minLeft: 150, sizeLeft: 300, maxLeft: 550,
            anchorToWindow: true,
            accessKey: "L"
        });
    }
...
function GetWebControls_Null(element) {
    var reVal = "";
    $(element).find('input,select,textarea').each(function (r) {
        var id = $(this).attr('id');
        var value = $(this).val();
...
function GetWebControls_Pic(element) {
    var reVal = "";
    $(element).find('input,select,textarea').each(function (r) {
        var id = $(this).attr('id');
        if (id != "imagepath") {
...
/*
弹出对话框（带：应用按钮、取消按钮、确定按钮）
*/
function configDialog(url, _id, _title, _width, _height, callBack, callBack1) {
    Loading(true);
    top.$.dialog({
        id: _id,
        width: _width,
...
NameFild:显示字段
*/
function MyJsonBindDrop(ControlId, Memo, ValueFild, NameFild, DataJson) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value=''>" + Memo + "</option>");
    }
    $.each(DataJson, function (i) {
        $(ControlId).append($("<option></option>").val(DataJson[i][ValueFild]).html(DataJson[i][NameFild]));
    });
}
/*
验证对象值不能重复 
...
function imageUnFormat(cellvalue, options, cell) {
    if ($('img', cell).attr('src') == '../../Content/Images/checkokmark.gif') return '1';
    if ($('img', cell).attr('src') == '../../Content/Images/checknomark.gif') return '0';
}
//搜索条件为全部时隐藏输入框
...
keyfield：主键Id、
Msg：提示消息*/
function CheckExist(id, keyfield, Msg, url) {
    var $ObjFiel = $("#" + id);
    var $KeyFiel = $("#" + keyfield);
    var postData = {
        fieldname: id,
        fieldvalue: $ObjFiel.val(),
        keyvalue: $KeyFiel.val()
    }
    getAjax(url, postData, function (data) {
        if (data.toLocaleLowerCase() == 'false') {
            tipCss("#" + id, Msg + "已存在,请重新输入");
        }
        Loading(false);
...
function IsChecked(id, content) {
    ...
    if (content == "" || content == undefined) {
        ...
        if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
            isOK = false;
            tipDialog('您没有选中' + content + '任何项,请您选中后再操作。', 4, 'warning');
        } else if (id.split(",").length > 1) {
            isOK = false;
            tipDialog('很抱歉,一次只能选择一条' + content + '记录。', 4, 'warning');
        }
    }
    return isOK;
}
function IsDelData(id, content) {
    var isOK = true;
    if (content == "" || content == undefined) {
        if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
            isOK = false;
