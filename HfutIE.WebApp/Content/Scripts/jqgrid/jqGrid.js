    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            updatepager = function (rn, dnd) {
                ...
                if (pgboxes) {
                    ...
                    if (ts.p.pginput === true) {
                        ...
                        var sumpage = $.fmatter ? $.fmatter.util.NumberFormat(ts.p.lastpage, fmt) : ts.p.lastpage;
                        $(sppg).html(sumpage.replace(/ /g, ''));
                    ...
                    if (ts.p.viewrecords) {
                        if (ts.p.reccount === 0) {
                            ...
                            var _totaltime = Number(ts.p.totaltime) + Number(ts.p.costtime);
                            $(".ui-paging-info", pgboxes).html($.jgrid.format(ts.p.recordtext, from.replace(/ /g, ''), to.replace(/ /g, ''), tot.replace(/ /g, ''), _totaltime));
                        }
                    }
            ...
            endReq = function () {
                ...
                var jgrid = $.jgrid.jqID(ts.p.id);
                var $jqgrid_bdiv = $("#gview_" + ts.p.id).find('.ui-jqgrid-bdiv');
                        ...
                        case "script":
                            $.ajax($.extend({
                                ...
                                success: function (data, st, xhr) {
                                    if (data == null) { endReq(); }
                                    if (data != null && data.costtime != null) {
                                        ts.p.costtime = data.costtime;//加载的时候 查询耗时
                                    }
            ...
            setPager = function (pgid, tp) {
                ...
                $("#" + $.jgrid.jqID(pgid))
                .append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td><td id='" + lft + "' align='left'></td></tr></tbody></table></div>")
                ...
                if (ts.p.pgbuttons === true) {
                    ...
                    pgl += "<td id='" + po[0] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-first'></span></td>";
                    pgl += "<td id='" + po[1] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-prev'></span></td>";
                    pgl += pginp !== "" ? sep + pginp + sep : "";
                    pgl += "<td id='" + po[2] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-next'></span></td>";
                    pgl += "<td id='" + po[3] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-end'></span></td>";
                ...
                $("#testpg").remove();
                if (twd > 0) {
                    if (pginp !== "") { twd += 10; } //should be param
                    $("td#" + pgid + "_" + ts.p.pagerpos, "#" + pgcnt).width(twd - 4);
                }
                ts.p._nvtd = [];
            ...
            //imgs = "<span class='s-ico' style='display:none'><span class='jq-col-sort-icon'></span></span>";
            imgs = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc" + iac + " ui-state-disabled ui-sort-" + dir + "'></span>";
            imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-state-disabled ui-sort-" + dir + "'></span></span>";
            if (ts.p.multiSort) {
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                var align = 'left';
                if (ts.p.colModel[i].align) {
                    align = ts.p.colModel[i].align;
                }
                thead += "<div style='text-align:" + align + "' id='jqgh_" + ts.p.id + "_" + ts.p.colModel[i].name + "' " + tdc + ">" + ts.p.colNames[i];
        ...
        getGridRowById: function (rowid) {
            ...
            this.each(function () {
                try {
                    ...
                    while (i--) {
                        if (rowid !=null && rowid.toString() === this.rows[i].id) {
                            row = this.rows[i];
                            break;
                        }
                    }
        ...
        addRowData: function (rowid, rdata, pos, src) {
            ...
            if (rdata) {
                ...
                this.each(function () {
                    ...
                    $("#gview_" + t.p.id).find('.ui-jqgrid-bdiv').find('.jqgridNotdata').remove();
