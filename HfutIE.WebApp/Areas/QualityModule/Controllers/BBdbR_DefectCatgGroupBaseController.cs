...
namespace HfutIE.WebApp.Areas.QualityModule.Controllers
{
    ...
    /// 缺陷类别分组基本信息表控制器
    /// </summary>
    public class BBdbR_DefectCatgGroupBaseController : PublicController<BBdbR_DefectCatgGroupBase>
    {
        #region 创建数据库操作对象区域
        BBdbR_DefectCatgGroupBaseBll MyBll = new BBdbR_DefectCatgGroupBaseBll(); //===复制时需要修改===
        #endregion
        ...
        public ActionResult GridListJson(string sort, string areaId, string DefectCatgGroupCd, string DefectCatgGroupNm, JqGridParam jqgridparam)
        {
            try
            {
                ...
                strSql.Append(@"select a.*,b.DefectCatgCd,b.DefectCatgNm from BBdbR_DefectCatgGroupBase a left join BBdbR_DefectCatgBase b on a.DefectCatgId = b.DefectCatgId where a.Enabled =1 and b.Enabled =1 ");
        ...
        #region 3.提交新增表单
        public ActionResult Submit(string KeyValue, string Catgid, BBdbR_DefectCatgGroupBase entity)
        {
            try
            {
                int IsOk = 0;//用于判断
                ...
                {
                    //===复制时需要修改===
                    //BBdbR_DefectCatgGroupBase Oldentity1 = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
                    BBdbR_DefectCatgGroupBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);
        ...
        public ActionResult GridPageByCondition(string sort, string areaId, string DefectCatgGroupCd, string DefectCatgGroupNm, JqGridParam jqgridparam)
        {
            try
            {
                #region 原方法
                //Stopwatch watch = CommonHelper.TimerStart();
                //List<BBdbR_DefectCatgGroupBase> ListData;
        ...
        public ActionResult GetExcel_Data(string sort, string areaId, string DefectCatgGroupCd, string DefectCatgGroupNm, JqGridParam jqgridparam)
        {
            try
            {
                ...
                StringBuilder strSql = new StringBuilder();
                strSql.Append(@"select a.DefectCatgGroupCd,a.DefectCatgGroupNm,b.DefectCatgNm,a.Dsc,a.CreTm,a.CreNm,a.MdfTm,a.MdfNm,a.Rem from BBdbR_DefectCatgGroupBase a left join BBdbR_DefectCatgBase b on a.DefectCatgId = b.DefectCatgId where a.Enabled =1 and b.Enabled =1 ");
                if (areaId != "all" && areaId != "undefined"&& areaId!="")
                {
                    strSql.Append(" and a.DefectCatgId = '" + areaId + "' ");
                }
        ...
        public ActionResult GetDefectCatgList()
        {
            try
            {
                #region 
                StringBuilder strSql = new StringBuilder();
                strSql.Append(@"select * from BBdbR_DefectCatgBase where Enabled =1 order by DefectCatgNm");
                DataTable dt = DataFactory.Database().FindTableBySql(strSql.ToString(), false);
                var JsonData = new
                {
                    rows = dt,
