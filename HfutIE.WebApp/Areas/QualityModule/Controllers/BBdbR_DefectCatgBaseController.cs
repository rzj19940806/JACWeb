...
namespace HfutIE.WebApp.Areas.QualityModule.Controllers
{
    ...
    public class BBdbR_DefectCatgBaseController : PublicController<BBdbR_DefectCatgBase>
    {
        ...
        BBdbR_DefectCatgBaseBll MyBll = new BBdbR_DefectCatgBaseBll(); //===复制时需要修改==       
        #endregion

        #region 方法区
        #region 1.新增编辑
        public ActionResult SubmitForm1(string KeyValue, BBdbR_DefectCatgBase entity)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                ...
                {
                    //===复制时需要修改===
                    BBdbR_DefectCatgBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
        ...
        public ActionResult GetGridPageJson(string DefectCatgCd, string DefectCatgNm, JqGridParam jqgridparam)
        {
            try
            {
                ...
                strSql.Append(@"select * from BBdbR_DefectCatgBase where Enabled = 1 ");
        ...
        public ActionResult GetExcel_Data(string DefectCatgCd, string DefectCatgNm, JqGridParam jqgridparam)
        {
            try
            {
                ...
                StringBuilder strSql = new StringBuilder();
                strSql.Append(@"select DefectCatgCd,DefectCatgNm,Dsc,CreTm,CreNm,MdfTm,MdfNm,Rem from BBdbR_DefectCatgBase where Enabled = 1 ");
                #region 判断输入框内容添加检索条件
                ////是否加缺陷类型编号模糊搜索
                //if (DefectCatgCd != "" && DefectCatgCd != null)
                //{
