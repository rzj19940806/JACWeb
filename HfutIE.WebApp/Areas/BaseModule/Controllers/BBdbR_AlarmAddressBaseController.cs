...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    public class BBdbR_AlarmAddressBaseController : PublicController<BBdbR_AlarmAddressBase>
    {
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_AlarmAddressBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                ...
                {
                    //===复制时需要修改===
                    BBdbR_AlarmAddressBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
        ...
        public override ActionResult Delete(string KeyValue, string ParentId, string DeleteMark)
        {
            ...
            try
            {
                ...
                if (array != null && array.Length > 0)
                {
                    ...
                    MyBll.Delete(array);//删// MyBll.Delete(array[i]);
                    IsOk = 1;
        ...
        #region 5.编辑填充界面数据
        public ActionResult SetAlarmForm(string KeyValue)
        {
            try
            {
                DataTable ListData = MyBll.SetAlarmInfor(KeyValue);
                var JsonData = new
                {
                    rows = ListData,
                };
        ...
        public ActionResult ImportExel()
        {
            ...
            List<BBdbR_AlarmAddressBase> BBdbRAlarmAddressBaseList = new List<BBdbR_AlarmAddressBase>();
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (dt.Rows[i]["报警地址"].ToString().Trim() != "" && dt.Rows[i]["报警路径"].ToString().Trim() != "" && dt.Rows[i]["报警等级"].ToString().Trim() != "")
                            {
                                ...
                                {
                                    IsOk = IsOk + b;
                                    BBdbRAlarmAddressBaseList.Clear();
                                }
                                else
                                {
                                    dr = Newdt.NewRow();
