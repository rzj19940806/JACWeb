...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    /// 铭牌信息表控制器
    /// </summary>
    public class MBB_BD_PRODUController : PublicController<MBB_BD_PRODU>
    {
        #region 全局变量定义区
        ...
        #region 创建数据库操作对象区域
        MBB_BD_PRODUBll MyBll = new MBB_BD_PRODUBll(); //===复制时需要修改===
        #endregion
        ...
        public ActionResult GridPage(JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<MBB_BD_PRODU> ListData = MyBll.GetPageList(jqgridparam);    //===复制时需要修改===
                var JsonData = new
                {
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(MBB_BD_PRODU entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                string Name = "CODE";        //页面中的编号字段名，如：公司编号   //===复制时需要修改===
                string Value = entity.CODE;  //页面中的编号字段值                 //===复制时需要修改===
                string Message = KeyValue == "" ? "新增成功。" : "编辑成功。";

                if (!string.IsNullOrEmpty(KeyValue))//编辑操作
                {
                    //===复制时需要修改===
                    MBB_BD_PRODU Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
                    entity.GID = KeyValue;
        ...
        /// <returns></returns>
        public ActionResult DeleteNamePlate(string KeyValue)
        {
        ...
        public ActionResult GridPageJsonFAS(string keywords, string Condition, JqGridParam jqgridparam)
        {
            try
            {
                ...
                Stopwatch watch = CommonHelper.TimerStart();
                List<MBB_BD_PRODU> ListData = MyBll.GetPageListByCondition(keyword, Condition, jqgridparam);//===复制时需要修改===
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
