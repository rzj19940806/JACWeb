...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    /// 公司基础信息表控制器
    /// </summary>
    public class E_PressureRecordInfoController : PublicController<E_PressureRecordInfo>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "E_PressureRecordInfo";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_CompanyBase> repository_BBdbR_CompanyBase = new RepositoryFactory<BBdbR_CompanyBase>();
        #endregion

        #region 创建数据库操作对象区域
        //E_ProduceInfoRecordBll MyBll = new E_ProduceInfoRecordBll(); //===复制时需要修改===
        #endregion
