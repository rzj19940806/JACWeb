...
{
    /// <summary>
    /// BBdbR_GetItemBase控制器
    /// </summary>
    public class BBdbR_GetItemBaseController : PublicController<BBdbR_GetItemBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_GetItemBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_GetItemBase> repository_getitembase = new RepositoryFactory<BBdbR_GetItemBase>();
        #endregion

        #region 创建数据库操作对象区域
        BBdbR_GetItemBaseBll MyBll = new BBdbR_GetItemBaseBll(); //===复制时需要修改===
        #endregion
        ...
        public ActionResult TreeJson()
        {
            try
            {
                ...
                if (DataHelper.IsExistRows(dt))
                {
                    ...
                    {
                        string area_key = row["Id"].ToString();
        ...
        /// <returns></returns>
        public ActionResult GridListJson(string area_key, string parentId, JqGridParam jqgridparam)
        {

            List<BBdbR_GetItemBase> ListData = MyBll.GetPlanList(area_key, parentId);
            var JsonData = new
            {
                rows = ListData,
            };
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_GetItemBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                string Name = "GetItemCd";        //页面中的编号字段名，如：公司编号   //===复制时需要修改===
                string Value = entity.GetItemCd;  //页面中的编号字段值                 //===复制时需要修改===
        ...
        public override ActionResult Delete(string KeyValue, string ParentId, string DeleteMark)
        {
            ...
            try
            {
                ...
                if (array != null && array.Length > 0)
                {
                    ...
                    {
                        repository_getitembase.Repository().Delete(array[i]);//删
                        IsOk = 1;
                    }
                }
                if (IsOk > 0)//表示删除成
        ...
        public ActionResult GridPageByCondition(string keywords, string Condition, JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<BBdbR_GetItemBase> ListData = MyBll.GetPageListByCondition(keyword, Condition, jqgridparam);//===复制时需要修改===
                var JsonData = new
                {
        ...
        #region 8.编辑获取实体信息
        public ActionResult SetGetItemForm(string keyvalue) //主键
        {
            try
            {
                BBdbR_GetItemBase ListData = MyBll.SetGetItemForm(keyvalue);
                var JsonData = new
                {
                    rows = ListData,
                };
        ...
        public ActionResult ImportExel(string area_key, string area_name)
        {
            ...
            List<BBdbR_GetItemBase> CntlAddrList = new List<BBdbR_GetItemBase>();
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (dt.Rows[i]["采集项编号"].ToString().Trim() != "")
                        {
                            ...
                            if (DeviceCount > 0)
                            {
                                dr = Newdt.NewRow();
                                dr[0] = errorNum;
                                dr[1] = "第[" + dt.Rows[i]["rowid"].ToString() + "]行[采集项编号]";
                                dr[2] = "在系统中已存在不能重复插入";
                                Newdt.Rows.Add(dr);
                                errorNum++;
                                IsCheck = 0;
