...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    /// </summary>
    public class BBdbR_FaultBaseController : PublicController<BBdbR_FaultBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_FaultBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_FaultBase> repository_faultbase = new RepositoryFactory<BBdbR_FaultBase>();
        #endregion

        #region 创建数据库操作对象区域
        BBdbR_FaultBaseBll MyBll = new BBdbR_FaultBaseBll(); //===复制时需要修改===
        #endregion
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_FaultBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                string Name = "FaultCd";        //页面中的编号字段名，如：公司编号   //===复制时需要修改===
                string Value = entity.FaultCd;  //页面中的编号字段值                 //===复制时需要修改===
                string Message = KeyValue == "" ? "新增成功。" : "编辑成功。";

                if (!string.IsNullOrEmpty(KeyValue))//编辑操作
                {
                    //===复制时需要修改===
                    BBdbR_FaultBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
                    entity.FaultId = KeyValue;
        ...
        public ActionResult GridPageByCondition(string keywords, string Condition, JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<BBdbR_FaultBase> ListData = MyBll.GetPageListByCondition(keyword, Condition, jqgridparam);//===复制时需要修改===
                var JsonData = new
                {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GridFaultJson()
        {
            try
            {
                List<BBdbR_FaultBase> ListData = MyBll.GetFaultList();
                var JsonData = new
                {
                    rows = ListData,
                };
        ...
        public ActionResult ImportExel()
        {
            ...
            List<BBdbR_FaultBase> BFacRShiftBaseList = new List<BBdbR_FaultBase>();
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (dt.Rows[i]["故障编号"].ToString().Trim() != "" && dt.Rows[i]["故障名称"].ToString().Trim() != "")//&& dt.Rows[i]["时间类型"].ToString().Trim() != "" && dt.Rows[i]["开始时间"].ToString().Trim() != "" && dt.Rows[i]["结束时间"].ToString().Trim() != ""
                            {
                                ...
                                if (DeviceCount > 0)
                                {
                                    dr = Newdt.NewRow();
                                    dr[0] = errorNum;
                                    dr[1] = "第[" + dt.Rows[i]["rowid"].ToString() + "]行[故障编号]";
                                    dr[2] = "在系统中已存在不能重复插入";
                                    Newdt.Rows.Add(dr);
                                    errorNum++;
                                    IsCheck = 0;
                                ...
                                if (DeviceCount > 0)
                                {
                                    dr = Newdt.NewRow();
                                    dr[0] = errorNum;
                                    dr[1] = "第[" + dt.Rows[i]["rowid"].ToString() + "]行[故障名称]";
                                    dr[2] = "在系统中已存在不能重复插入";
                                    Newdt.Rows.Add(dr);
                                    errorNum++;
                                    IsCheck = 0;
