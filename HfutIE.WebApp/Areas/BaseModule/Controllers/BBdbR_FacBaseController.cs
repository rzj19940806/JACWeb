...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    /// </summary>
    public class BBdbR_FacBaseController : PublicController<BBdbR_FacBase>
    {
        #region 全局变量定义区

        //定义本页面主要操作的表的表名，称为主表
        string tableName = "BBdbR_FacBase";//===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_FacBase> repository_facbase = new RepositoryFactory<BBdbR_FacBase>();
        #endregion

        #region 创建数据库操作对象区域
        //BBdbR_FacBaseBll，用以访问BBdbR_FacBaseBll中操作数据库的方法
        BBdbR_FacBaseBll MyBll = new BBdbR_FacBaseBll(); //===复制时需要修改===
        BBdbR_StfBaseBll MyBll1 = new BBdbR_StfBaseBll(); //===复制时需要修改===
        #endregion
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_FacBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                ...
                {  
                    //===复制时需要修改===
                    BBdbR_FacBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
        ...
        {
            BBdbR_WorkshopBaseBll WorkShopBll = new BBdbR_WorkshopBaseBll();
            ...
            try
            {
                ...
                int a = WorkShopBll.GetWorkshopCount(KeyValue);//查找工厂基础信息中是否有在本公司名下，如果有就不可以删除
                if (a > 0)
                {
        ...
        public override ActionResult SetForm(string KeyValue)
        {
            try
            {
                BBdbR_FacBase Deviceentity = MyBll.GetPlanList(KeyValue);
        ...
        public ActionResult GridPageJson1()
        {
            try
            {
                List<BBdbR_FacBase> ListData = MyBll.GetPlanList();
                var JsonData = new
                {
                    rows = ListData,
                };
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult FactoryTypeJson()
        {
            try
            {
                List<BBdbR_FacBase> ListData = MyBll.GetTypeList();
                var JsonData = new
                {
                    rows = ListData,
                };
