...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    public class BBdbR_CarStrandedBaseController : PublicController<BBdbR_CarStrandedBase>
    {
        ...
        BBdbR_CarStrandedBaseBll MyBll = new BBdbR_CarStrandedBaseBll(); //===复制时需要修改===MyBll.GetPlanList("");
        public readonly RepositoryFactory<BBdbR_CarStrandedBase> repository_carstrandedbase = new RepositoryFactory<BBdbR_CarStrandedBase>();
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_CarStrandedBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
        ...
        /// <returns></returns>
        public override ActionResult Delete(string KeyValue, string RuleId, string DeleteMark)
        {
            ...
            try
            {
                ...
                if (array != null && array.Length > 0)
                {
                    ...
                    {
                        repository_carstrandedbase.Repository().Delete(array[i]);//删
                        IsOk = 1;
                    }
                }
                if (IsOk > 0)//表示删除成
        ...
        #endregion
        #region 7.结束AVI站点下拉框
        public ActionResult GetPlineNm1()
        {
            try
            {
                DataTable dataTable = MyBll.GetPlineNm1();
                var JsonData = new
                {
                    rows = dataTable,
                };
