...
namespace HfutIE.WebApp.Areas.BaseModule.Controllers
{
    ...
    /// 质控点基本表控制器
    /// </summary>
    public class BBdbR_QualityCheckPointBaseController : PublicController<BBdbR_QualityCheckPointBase>
    {
        #region 创建数据库操作对象区域
        //创建数据库访问对象，用以访问其中操作数据库的方法
        BBdbR_QualityCheckPointBaseBll MyBll = new BBdbR_QualityCheckPointBaseBll(); //===复制时需要修改===
        BBdbR_QualityCheckModelBaseBll MyBll1 = new BBdbR_QualityCheckModelBaseBll(); //===复制时需要修改===
        public readonly RepositoryFactory<BBdbR_QualityCheckPointBase> repository_avibase = new RepositoryFactory<BBdbR_QualityCheckPointBase>();
        #endregion
        ...
        public ActionResult GridPage(JqGridParam jqgridparam)
        {
            try
            {
                ...
                List<BBdbR_QualityCheckPointBase> ListData = MyBll.GetPageList(jqgridparam);    //===复制时需要修改===
                var JsonData = new
                {
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(BBdbR_QualityCheckPointBase entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                string Name = "QualityCheckPointCd";        //页面中的编号字段名，如：公司编号   //===复制时需要修改===
                string Value = entity.QualityCheckPointCd;  //页面中的编号字段值                 //===复制时需要修改===\
                string Message = "";
                if (KeyValue == "")
                {
                    Message = "新增成功。";
                ...
                {
                    //===复制时需要修改===
                    BBdbR_QualityCheckPointBase Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
                ...
                else//新增操作
                {
                    ...
                    entity.QueueNo= MyBll.GetMaxNo("QueueNo");
                    entity.Create();
        ...
        public override ActionResult Delete(string KeyValue, string ParentId, string DeleteMark)
        {
            ...
            try
            {
                ...
                string Name = "QualityCheckPointId";
                for (int i = 0; i < array.Length; i++)
                {
                    string Value = array[i];
                    int IsOk1 = MyBll1.CheckCount(Name, Value);//判断该质控点下是否有配置检查模板
                    if (IsOk1 > 0)//存在
                    {
                        Message = "该节点下有配置检查模板不可删除！";
                        return Content(new JsonMessage { Success = false, Code = IsOk.ToString(), Message = Message }.ToString());
                    }
                }
        ...
        public ActionResult GridPageByCondition(string keywords, string Condition, JqGridParam jqgridparam)
        {
            try
            {
                ...
                Stopwatch watch = CommonHelper.TimerStart();
                List<BBdbR_QualityCheckPointBase> ListData = MyBll.GetPageListByCondition(keyword, Condition, jqgridparam);//===复制时需要修改===
                var JsonData = new
                {
                    total = jqgridparam.total,
                    page = jqgridparam.page,
        ...
        public ActionResult ImportExel()
        {
            ...
            List<BBdbR_QualityCheckPointBase> EntityList = new List<BBdbR_QualityCheckPointBase>();
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        //int queueno;
                        //if (int.TryParse(dt.Rows[i]["顺序号"].ToString(), out queueno) == false)
                        //{
                        //    dr = Newdt.NewRow();
                        //    dr[0] = errorNum;
                        //    dr[1] = "第[" + dt.Rows[i]["rowid"].ToString() + "]行[顺序号]";
                        //    dr[2] = "数字格式不正确请重新输入";
                        //    Newdt.Rows.Add(dr);
                        //    errorNum++;
                        //    IsCheck = 0;
                        ...
                        if (dt.Rows[i]["质控点编号"].ToString().Trim() != "")
                        {
                            int Count = MyBll.CheckCount("QualityCheckPointCd", dt.Rows[i]["质控点编号"].ToString().Trim());//是否有相同编号
                            if (Count > 0)
                            {
                                dr = Newdt.NewRow();
                                dr[0] = errorNum;
                                dr[1] = "第[" + dt.Rows[i]["rowid"].ToString() + "]行[质控点编号]";
                                dr[2] = "在系统中已存在不能重复插入";
                                Newdt.Rows.Add(dr);
                                errorNum++;
                                IsCheck = 0;
                            ...
                            else
                            {
                                ...
                                BBdbR_QualityCheckPointBase entity = new BBdbR_QualityCheckPointBase();
                                entity.QualityCheckPointCd = dt.Rows[i]["质控点编号"].ToString().Trim();
                                entity.QualityCheckPointNm = dt.Rows[i]["质控点名称"].ToString().Trim();
                                entity.QueueNo = MyBll.GetMaxNo("QueueNo");
                                entity.QualityCheckPointDsc = dt.Rows[i]["质控点描述"].ToString().Trim();
