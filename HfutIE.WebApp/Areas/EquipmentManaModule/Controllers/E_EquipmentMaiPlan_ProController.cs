...
{
    /// <summary>
    /// E_EquipmentMaiPlan_Pro控制器
    /// </summary>
    public class E_EquipmentMaiPlan_ProController : PublicController<E_EquipmentMaiPlan_Pro>
    {
        #region 创建数据库操作对象区域
        E_EquipmentMaiPlan_ProBll MyBll = new E_EquipmentMaiPlan_ProBll();//===复制时需要修改===
        public readonly RepositoryFactory<E_EquipmentMaiPlan_Pro> repository_equipmentmaiplan_pro = new RepositoryFactory<E_EquipmentMaiPlan_Pro>();
        #endregion
        #region 方法区
        #region 1.新增编辑方法
        ...
        //不存在再进行下一步
        public override ActionResult SubmitForm(E_EquipmentMaiPlan_Pro entity, string KeyValue)//===复制时需要修改===
        {
            try
            {
                int IsOk = 0;//用于判断
                string Name = "MaiPlanCode";        //页面中的编号字段名，如：公司编号   //===复制时需要修改===
                string Value = entity.MaiPlanCode;  //页面中的编号字段值                 //===复制时需要修改===
                string Message = KeyValue == "" ? "新增成功。" : "编辑成功。";

                if (!string.IsNullOrEmpty(KeyValue))//编辑操作
                {
                    //===复制时需要修改===
                    E_EquipmentMaiPlan_Pro Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetStfNm()
        {
            try
            {
                DataTable dataTable = MyBll.GetStfNm();
                var JsonData = new
                {
                    rows = dataTable,
                };
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetDvcCd(string DvcNm)
        {
            try
            {
                DataTable dataTable = MyBll.GetDvcCd(DvcNm);
                var JsonData = new
                {
                    rows = dataTable,
                };
        ...
        public ActionResult ImportExel()
        {
            ...
            List<E_EquipmentMaiPlan_Pro> E_EquipmentMaiPlan_ProList = new List<E_EquipmentMaiPlan_Pro>();
                ...
                if (file != null)
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (dt.Rows[i]["维保计划编号"].ToString().Trim() != "" && dt.Rows[i]["维保计划名称"].ToString().Trim() != "" && dt.Rows[i]["设备编号"].ToString().Trim() != "" && dt.Rows[i]["设备名称"].ToString().Trim() != "")
                            {
                                E_EquipmentMaiPlan_Pro E_EquipmentMaiPlan_ProEntity = new E_EquipmentMaiPlan_Pro();
                                string[] MaiPlanCatg = dt.Rows[i]["维护类别"].ToString().Trim().Split(',');
                                foreach (string Catg in MaiPlanCatg)
                                {
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanId = System.Guid.NewGuid().ToString();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanCode = dt.Rows[i]["维保计划编号"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanName = dt.Rows[i]["维保计划名称"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.DvcCd = dt.Rows[i]["设备编号"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.DvcNm = dt.Rows[i]["设备名称"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.CarPartId = dt.Rows[i]["设备部件名称"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanCatg = Catg;
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanType = dt.Rows[i]["维护类型"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanCon = dt.Rows[i]["维护内容"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanStandard = dt.Rows[i]["维修标准"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanCycle = dt.Rows[i]["维护周期"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanSta = dt.Rows[i]["维护状态"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.PlineId = dt.Rows[i]["责任产线"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.StfId = dt.Rows[i]["责任人"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.Frequency = dt.Rows[i]["频次"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.MaiPlanPer = dt.Rows[i]["维护人员"].ToString().Trim();
                                    //E_EquipmentMaiPlan_ProEntity.MaiPlanTime = dt.Rows[i]["维护时间"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.Rem = dt.Rows[i]["备注"].ToString().Trim();
                                    E_EquipmentMaiPlan_ProEntity.Create();
                                    E_EquipmentMaiPlan_ProList.Add(E_EquipmentMaiPlan_ProEntity);
                                    int b = database.Insert(E_EquipmentMaiPlan_ProList);
                                    if (b > 0)
                                    {
                                        IsOk = IsOk + b;
                                        E_EquipmentMaiPlan_ProList.Clear();
                                    }
                                    else
                                    {
                                        dr = Newdt.NewRow();
