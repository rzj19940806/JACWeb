...
namespace HfutIE.WebApp.Areas.QualityAdd_HTModule.Controllers
{
    ...
    /// 缺陷明细信息表控制器
    /// </summary>
    public class BBdbR_DefectCatgDeTail_AddController : PublicController<BBdbR_DefectCatgDeTail_Add>
    {
        #region 创建数据库操作对象区域
        BBdbR_DefectCatgDeTail_AddBll MyBll = new BBdbR_DefectCatgDeTail_AddBll(); //===复制时需要修改===
        #endregion
        ...
        public ActionResult TreeJson()
        {
            try
            {
                DataTable dt = MyBll.GetTree("CH");//获取树所需数据
        ...
        public ActionResult GridListJson(string areaId, string sort, string DefectCd, string DefectNm, JqGridParam jqgridparam)
        {
            try
            {
                ...
                {
                    strSql.Append(@"select b.DefectCatgNm,b.DefectCatgCd,c.DefectCatgGroupCd,c.DefectCatgGroupNm,a.* from BBdbR_DefectCatgDeTail_Add a left join BBdbR_DefectCatgBase_Add b on a.DefectCatgId = b.DefectCatgId left join  BBdbR_DefectCatgGroupBase_Add c on a.DefectCatgGroupId = c.DefectCatgGroupId where a.Enabled=1 and b.Enabled =1 and c.Enabled =1 and b.Type='CH'");
                }
                else
                {
                    if (areaId == "all")
                    ...
                    {
                        strSql.Append(@"select b.DefectCatgNm,b.DefectCatgCd,c.DefectCatgGroupCd,c.DefectCatgGroupNm,a.* from BBdbR_DefectCatgDeTail_Add a left join BBdbR_DefectCatgBase_Add b on a.DefectCatgId = b.DefectCatgId left join  BBdbR_DefectCatgGroupBase_Add c on a.DefectCatgGroupId = c.DefectCatgGroupId where a.Enabled=1 and b.Enabled =1 and c.Enabled =1 and a.DefectCatgId = '" + areaId + "' and b.Type='CH'");
                    }
                    else//缺陷分组节点
                    {
                        strSql.Append(@"select b.DefectCatgNm,b.DefectCatgCd,c.DefectCatgGroupCd,c.DefectCatgGroupNm,a.* from BBdbR_DefectCatgDeTail_Add a left join BBdbR_DefectCatgBase_Add b on a.DefectCatgId = b.DefectCatgId left join  BBdbR_DefectCatgGroupBase_Add c on a.DefectCatgGroupId = c.DefectCatgGroupId where a.Enabled=1 and b.Enabled =1 and c.Enabled =1 and a.DefectCatgGroupId = '" + areaId + "' and b.Type='CH'");
                    }
                }
        ...
        public ActionResult GridPageByCondition(string areaId, string sort, string DefectCd, string DefectNm, JqGridParam jqgridparam)
        {
            try
            {
                #region 原方法
                //Stopwatch watch = CommonHelper.TimerStart();
                //List<BBdbR_DefectCatgDeTail_Add> ListData;
        ...
        #region 4.提交新增编辑表单
        public ActionResult Submit(string KeyValue, string catggroupid, BBdbR_DefectCatgDeTail_Add entity)
        {
            try
            {
                int IsOk = 0;//用于判断
                ...
                {
                    //===复制时需要修改===
                    BBdbR_DefectCatgDeTail_Add Oldentity1 = repositoryfactory.Repository().FindEntity(KeyValue);//获取没更新之前实体对象
                    BBdbR_DefectCatgDeTail_Add Oldentity = repositoryfactory.Repository().FindEntity(KeyValue);
