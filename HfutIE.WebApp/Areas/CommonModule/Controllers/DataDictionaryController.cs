...
namespace HfutIE.WebApp.Areas.CommonModule.Controllers
{
    ...
    public class DataDictionaryController : PublicController<Base_DataDictionaryDetail>
    {
        ...
        /// <summary>
        /// 根据分类编码》获取数据字典明显列表（返回树JSON）
        /// </summary>
        /// <param name="Code">分类编码</param>
        /// <returns></returns>
        public ActionResult DetailJson(string Code)
        {
            List<Base_DataDictionaryDetail> list = base_datadictionarybll.GetDataDictionaryDetailListByCode(Code);
            List<TreeJsonEntity> TreeList = new List<TreeJsonEntity>();
            foreach (Base_DataDictionaryDetail item in list)
            {
                string DataDictionaryDetailId = item.DataDictionaryDetailId;
                bool hasChildren = false;
                List<Base_DataDictionaryDetail> childnode = list.FindAll(t => t.ParentId == DataDictionaryDetailId);
                if (childnode.Count > 0)
                {
                    hasChildren = true;
                }
